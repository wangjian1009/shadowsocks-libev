...
static int parse_json_int(const json_value *value)
{
	if (value->type == json_integer) {
		...
		LOGE("Need type :%d or %d, now get wrong type: %d", json_integer, json_boolean, value->type);
...
static char * parse_json_string(const json_value *value)
{
	if (value->type == json_string) {
		...
		LOGE("Need type :%d or %d, now get wrong type: %d", json_string, json_null, value->type);
		LOGE("Invalid config format.");
	}
	return 0;
}
